...
router.get("/:id", wrapAsync(async(req, res)=>{// Matching ke time  path ka pattern same hai ya nahi yahi check hota hai.
...
router.get("/:id/edit",wrapAsync(async (req, res) => { // views/show.ejs k anker tag seh yaha request ayaga. 
    let { id } = req.params;
